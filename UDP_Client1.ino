/*
  UDP Bidirectional Commmunication for Client 1 in a network of one server with three clients, which not all have to be present.
  No change necessary.
...
bool ConfigESP () {
  ...
  success &= sendCom("AT+CIPSTA=\"192.168.4.3\"", "OK");
  return success;
}
...
bool StartCon () {
  ...
  return sendCom("AT+CIPSTART=1,\"UDP\",\"192.168.4.1\",93,92", "OK");
